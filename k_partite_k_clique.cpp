...
bool KPartiteKClique::KPartiteGraph::select(KPartiteKClique::KPartiteGraph next){
    KPartiteGraph::Vertex* vpt = last_vertex();
    if (!vpt)
        return false;

    KPartiteGraph::Vertex& v = vpt[0];
...
KPartiteKClique::KPartiteKClique(const bool* const* incidences, const int n_vertices, const int* first_per_part, const int k){
    ...
    for (int i=0; i<k; i++)
        recursive_graphs[i] = KPartiteGraph(this, i==0);
    ...
    for (int i=0; i<n_vertices; i++){
        while ((current_part < k-1) && (i >= parts[current_part + 1]))
            current_part += 1;
        all_vertices[i] = KPartiteGraph::Vertex(recursive_graphs[0], incidences[i], n_vertices, current_part, i);
    }
